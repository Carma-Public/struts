...
public class VelocityDecoratorServlet extends VelocityViewServlet {
	 ...
	 * <p/>
	 * <p>
	 * NOTE: If no charset is specified in the default.contentType property (in your velocity.properties) and you have
	 * specified an output.encoding property, then that will be used as the charset for the default content-type of
	 * pages served by this servlet.
	 * </p>
	 *
	 * @param config
	...
	public void init(ServletConfig config) throws ServletException {
		...
		defaultContentType = (String) getVelocityProperty(VelocityView.CONTENT_TYPE_KEY,
				VelocityView.DEFAULT_CONTENT_TYPE);

		String encoding = (String) getVelocityProperty(RuntimeConstants.OUTPUT_ENCODING,
		...
		if (!VelocityView.DEFAULT_OUTPUT_ENCODING.equalsIgnoreCase(encoding)) {
			...
			if (index < 0) {
				...
				getVelocityView().getLog().warn("VelocityViewServlet: Charset was already "
						+ "specified in the Content-Type property.  " + "Output encoding property will be ignored.");
			}
		}

		getVelocityView().getLog().info("VelocityViewServlet: Default content-type is: " + defaultContentType);
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see org.apache.velocity.tools.view.VelocityViewServlet#handleRequest(javax.servlet.http.HttpServletRequest,
	 ...
	 * </pre>
	 * 
	 * where defaultContentType is set to the value of the default.contentType property, or "text/html" if that is not
	 * set.
	 * </p>
	 *
	 * @param request
